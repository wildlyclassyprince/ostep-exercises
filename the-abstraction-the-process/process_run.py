...
class scheduler:
    ...
    # program looks like this: c7,i,c1,i
    # which means compute for 7, then I/O, then compute for 1, then I/O
    def load_program(self, program):
        ...
        for line in program.split(","):
            ...
            if opcode == "c":  # compute
                ...
                logger.info(f"bad opcode {opcode} (should be c or i)")
                exit(1)
        return
    ...
    def load(self, program_description):
        ...
        if len(tmp) != 2:
            logger.info(
                f"Bad description ({program_description}): Must be number <x:y>"
            )
            logger.info("   where X is the number of instructions")
            logger.info(
    ...
    def get_instructions(self, pid, index):
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            if io_done:
                logger.info("%3d*" % clock_tick)
            else:
                logger.info("%3d " % clock_tick)
            for pid in range(len(self.proc_info)):
                if pid == self.curr_proc and instruction_to_execute != "":
                    logger.info("%14s" % ("RUN:" + instruction_to_execute))
                else:
                    logger.info("%14s" % (self.proc_info[pid][PROC_STATE]))

            # CPU output here: if no instruction executes, output to space, otherwise a 1
            ...
            if num_outstanding > 0:
                logger.info("%14s" % str(num_outstanding))
                io_busy += 1
            else:
                logger.info("%10s" % " ")
...
if not options.solve:
    logger.info("Produce a trace of what would happen when you run these processes:")
    for pid in range(s.get_num_processes()):
        logger.info("Process %d" % pid)
        for inst in range(s.get_num_instructions(pid)):
            logger.info("  %s" % s.get_instructions(pid, inst))
    logger.info("Important behaviors:")
    logger.info("  System will switch when ")
    if options.process_switch_behavior == SCHED_SWITCH_ON_IO:
        logger.info("  the current process is FINISHED or ISSUES AN IO")
    else:
        logger.info("  the current process is FINISHED")
    logger.info("  After IOs, the process issuing the IO will ")
